...
class OnlineTestCase(unittest.TestCase):
    ...
    def testReadSysInfo(self):
        ...
        assert res is not None and 'romVersion' in res
        if VERBOSE:
    ...
    def testReadNetSyncInfo(self):
        ...
        assert res is not None and 'hostName' in res
        if VERBOSE:
    ...
    def testReadStorageInfoAll(self):
        more = 1
        i = 0
        while more:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='pilot-link Python binding tests')
    parser.add_argument("-p", "--port", dest="pilotport",
